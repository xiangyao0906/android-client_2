...
import com.xinly.dendrobe.databinding.BaseInfoBinding
 ...
 * 完善资料
 */
class BasicInfoActivity : BaseActivity<BaseInfoBinding, BasicInfoViewModel>() {
    ...
    override fun initViewObservable() {
        ...
        viewModel?.requestSelPicture?.addOnPropertyChangedCallback(object : Observable.OnPropertyChangedCallback() {
            ...
            override fun onPropertyChanged(sender: Observable?, propertyId: Int) {
                RxPermissions(this@BasicInfoActivity)
                    .request(Manifest.permission.CAMERA)
                    .subscribe {
                        if (it) {
                            Matisse.from(this@BasicInfoActivity)
                                .choose(setOf(MimeType.JPEG, MimeType.PNG)) //选择图片类型
                                .capture(true) //可拍照
                                .countable(false) //记录文件选择顺序
                                .captureStrategy(CaptureStrategy(true, "${Platform.getPackageName(this@BasicInfoActivity)}.fileprovider"))
